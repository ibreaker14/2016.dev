...
function wplfta_create_menu() {
  ...
  add_menu_page('wordpress login form to anywhere  Plugin Settings', ' (Login Form)', 'administrator', 'wp_wplfta_setting', 'wplfta_settings_page','');
...
function register_wfltfa_settings() { 
  ...
  register_setting( 'wplfta-settings-group', 'wplfta_remember_me_text_e' );
...
<form method="post" action="options.php">
    ...
    <table class="form-table">
		...
		<tr valign="top">
        <th scope="row">Disable Remember Me</th>
        <td><input type="checkbox" name="wplfta_remember_me_text_e"  value="1" <?php checked(get_option('wplfta_remember_me_text_e'), 1); ?>/>
